-- Add chat event triggers for automatic event posting
-- This will post events to chat when matches go live, scores are entered, and competitions are created/finished

-- Update chat_messages type constraint to include new event types
...
-- Function to post match live event
...
-- Function to post match score event
...
-- Function to post competition created event
...
BEGIN
  ...
  INSERT INTO chat_messages (
    ...
    NEW.group_id,
    (SELECT admin_id FROM groups WHERE id = NEW.group_id),
    'System',
...
-- Function to post competition finished event
...
-- Add deadline reminder function (to be called by a cron job or manually)
  ...
  LOOP
    ...
    IF days_left IN (3, 2, 1) THEN
      ...
      INSERT INTO chat_messages (
        ...
        comp.group_id,
        (SELECT admin_id FROM groups WHERE id = comp.group_id),
        'System',
