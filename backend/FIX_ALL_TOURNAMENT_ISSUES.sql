-- Complete Fix for All Tournament Issues
-- Run this script to fix:
-- 1. Chat messages showing 0 participants and 0 matches
...
-- Drop all existing competition notification triggers
...
-- Create new function that properly counts participants and matches
...
BEGIN
  ...
  -- Log for debugging
  RAISE NOTICE 'Competition %: % participants, % matches', NEW.id, participant_count, match_count;
...
-- This ensures participants and matches are already inserted
