-- Add tournament stage progression system
-- This will automatically create next stage matches when a stage is completed
...
BEGIN
  CASE current_stage
    ...
    WHEN 'final' THEN RETURN NULL; -- No next stage after final
    ELSE
      -- Determine initial stage based on participant count
...
BEGIN
  ...
  SELECT ARRAY_AGG(
    CASE 
      ...
      ELSE NULL -- This shouldn't happen in knockout, but handle it
    END
  ...
  IF v_next_stage = 'final' AND array_length(v_losers, 1) >= 2 THEN
    INSERT INTO matches (
      ...
      COALESCE(v_deadline, NOW() + INTERVAL '7 days'),
      'third_place',
      999 -- Special order for 3rd place match
    );
  END IF;
  ...
  SELECT 
    ...
    c.created_by,
    'System',
    'Next stage (' || v_next_stage || ') matches have been created!',
    'match_result',
    jsonb_build_object(
      'stage', v_next_stage,
