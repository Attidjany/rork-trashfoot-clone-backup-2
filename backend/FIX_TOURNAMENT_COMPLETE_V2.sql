-- ============================================
-- COMPLETE TOURNAMENT FIX V2
-- ============================================
-- This script fixes:
-- 1. Tournament bracket showing empty (missing stage/match_order in queries)
-- 2. Automatic stage progression when all matches complete
-- 3. Chat message showing correct participant/match counts
-- ============================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✓ Added stage column to matches';
  ELSE
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✓ Added match_order column to matches';
  ELSE
...
-- ============================================
-- PART 2: Fix Chat Message Trigger
-- ============================================
...
-- Create function that properly counts participants and matches
...
BEGIN
  ...
  SELECT STRING_AGG(p.name, ', ' ORDER BY p.name) INTO participant_names
  ...
  -- Log for debugging
  RAISE NOTICE 'Competition %: % participants, % matches, creator: %', 
    NEW.id, participant_count, match_count, creator_name;

  -- Insert chat message with actual counts
  INSERT INTO chat_messages (group_id, sender_id, sender_name, message, type, metadata, timestamp)
  VALUES (
    NEW.group_id,
    ...
    jsonb_build_object(
      ...
      'participantNames', COALESCE(participant_names, 'None'),
      'matchCount', match_count,
      'creatorName', COALESCE(creator_name, 'System'),
...
-- ============================================
-- PART 3: Tournament Stage Progression
-- ============================================

-- Drop existing tournament progression trigger
    ...
    IF next_stage IS NOT NULL THEN
      ...
      SELECT ARRAY_AGG(
        CASE
          ...
          ELSE home_player_id -- In case of draw, home player advances
        END
      ...
      IF current_stage = 'semi_final' THEN
        ...
        SELECT ARRAY_AGG(
          CASE
            WHEN home_score < away_score THEN home_player_id
            WHEN away_score < home_score THEN away_player_id
            ELSE away_player_id -- In case of draw, away player goes to 3rd place
          END
          ORDER BY match_order
        ) INTO losers
        FROM matches
...
SELECT 
  ...
  ) THEN '✓ competitions.created_by column exists'
  ELSE '✗ competitions.created_by column missing'
  END as check_7;
