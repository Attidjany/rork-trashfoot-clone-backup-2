-- ============================================
-- FIX TOURNAMENT COMPLETION LOGIC
-- Tournament should only complete when BOTH final and 3rd place are finished
-- ============================================
...
BEGIN
  ...
  IF v_current_stage = 'third_place' THEN
    ...
    -- Check if final is also completed
    SELECT EXISTS(
    ...
    -- If final is also completed, show podium and complete tournament
  ...
  IF v_current_stage = 'final' THEN
    ...
    IF v_final_winner_id IS NOT NULL THEN
      ...
      -- Check if third place is also completed
      SELECT EXISTS(
        SELECT 1 FROM matches
        WHERE competition_id = v_competition_id
        AND stage = 'third_place'
      ...
      -- Only complete tournament if 3rd place is also completed
      IF v_third_place_completed THEN
        -- Update competition status to completed
        UPDATE competitions
        SET status = 'completed'
        WHERE id = v_competition_id;
