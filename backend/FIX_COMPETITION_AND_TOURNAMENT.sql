-- ============================================
-- FIX COMPETITION CREATION MESSAGE & TOURNAMENT STAGE PROGRESSION
-- Apply this file in your Supabase SQL Editor
-- ============================================

-- PART 1: Fix Competition Created Message
-- ============================================

-- Update the trigger function to show participants and creator
...
BEGIN
  ...
  SELECT
    ...
    E'\n👥 Participants (' || participant_count || '): ' || COALESCE(participant_names, 'None') ||
    E'\n⚽ Matches: ' || match_count ||
    CASE 
      WHEN deadline_text IS NOT NULL THEN E'\n📅 Deadline: ' || deadline_text
      ELSE ''
    END,
    'competition_created',
    jsonb_build_object(
...
BEGIN
  ...
  INSERT INTO chat_messages (
    ...
    CASE v_next_stage
      ...
      ELSE v_next_stage
    END || 
    ') matches created: ' || (v_match_order - 1) || ' match(es) scheduled' ||
    CASE 
      WHEN v_next_stage = 'final' AND array_length(v_losers, 1) >= 2 THEN ' + 3rd place match'
      ELSE ''
    END,
    'match_result',
    jsonb_build_object(
